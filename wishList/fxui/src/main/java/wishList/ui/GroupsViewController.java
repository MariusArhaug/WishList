...
public class GroupsViewController extends AbstractController {
    ...
    private void updateFriendsView() {
        ...
        for (String member : group) {
            User friend = friends.stream().filter(e -> e.getEmail().equals(member)).findFirst().orElse(null);
            if (friend != null) {
    ...
    public void shareWishList(ActionEvent event) throws Exception {
        ...
        for(String s : stringGroup) {
            User groupMember = User.getUsers().stream().filter(e -> e.getEmail().equals(s)).findFirst().orElse(null);
        ...
        for (User u : group) {
            WishList alreadyShared = u.getInvitedWishLists().stream().filter(e -> e.getOwner().toString().equals(user.toString())
                    && e.getName().equals(wishListToShare.getName())).findFirst().orElse(null);
