...
- [**fxui**](../../wishList/fxui): Here lies javafx-code for the client as well as controllers for different _views_
...
> We realised pretty late that our foldername `wishList` did not comply with the CheckStyle's warning about foldernames, however this late in production a change to all these foldernames caused the application to run into wierd unexpected behaviour, we decided it was not worth to use too much time to fix this small detail for now, but we are planning on resolving it in the next sprint.
...
## Sprint 4 (plans for next iteration)

For our next iteration we want to fully utilize mocking in our FXUI tests aswell as refactoring the logic from **JsonHandler** into the **WishListService** on the server end. We also want to add to the functionality where user will be able to pich wishes that they want to cover from a friends wish list. We also want to implement two-way contact adding. In addition to this we want to expand upon our rest api test environment.


> Keep in mind that when we're talking about `Sprint 4`, that this only arbitrary, but we want to continue the agile thought process and in doing so, it is just as important to map out the next iteration's goals, to give the product owner an idea of the current state of the product but also getting every member of the team to be up to date with the current issues. 

## Diagrams and documentation

[**Documentation of rest service**](../../wishList/rest/README.md)

Sequence diagram capturing how to make and display a wish list in the app.

[**Sequence diagram**](../../wishList/WishListSequenceDiagram.puml)

Package diagram capturing how modules are connected.

[**Package diagram**](../../wishList/WishListPackageDiagram.puml)

Class diagram showcasing the classes in our app.

[**Class diagram**](../../wishList/classDiagram.puml)
